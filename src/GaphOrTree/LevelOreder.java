...
public class LevelOreder {
	...
	public static void main (String[] args) throws IOException{
        ...
        while(t > 0){
	    	...
	    	System.out.println("Width of th tree "+width);
	...
	private static int getWidth(Node root) {
		...
		Queue<Width> q=new LinkedList<Width>();
		q.add(new Width(0,root));
		...
		while(!q.isEmpty()) {
			...
			for(int i=0;i<size;i++) {
			    ...
			    if(p.node.left!=null) {			
			    	q.add(new Width( 2*(p.n-min)+1,p.node.left));
			    }
				if(p.node.right!=null) {
					q.add(new Width( 2*(p.n-min)+2,p.node.right));
				}
			}
			if(high-low+1>max) {
				max=high-low+1;
